...
def add_docx_table_from_df(container, df: pd.DataFrame, title: Optional[str] = None):
    """Render a pandas DataFrame into a DOCX table.

    `container` can be a `docx.document.Document` or a `docx.table._Cell`.
    """
    if title:
    ...
    table = container.add_table(rows=nrows + 1, cols=ncols + 1)
...
def md_table_to_df(md: str) -> Optional[pd.DataFrame]:
    """Parse a simple markdown table into a DataFrame.
...
def add_docx_table_any(container, obj, title: Optional[str] = None) -> bool:
    """Try to render obj (dict/list/markdown-table) as a real DOCX table."""
    if obj is None:
        return False
    # markdown table
    if isinstance(obj, str):
        df = md_table_to_df(obj)
        if df is not None:
            add_docx_table_from_df(container, df, title=title)
            return True
        return False
    ...
    if isinstance(obj, dict) and obj:
        try:
            ...
            if isinstance(first, list):
                ...
                ncol = len(first)
                cols = [f"C{i+1}" for i in range(ncol)]
                df = rows_list_to_df(rows, cols, obj)
            elif isinstance(first, dict):
                df = grid_to_df(obj)
            else:
                # not a grid; render as key/value
                df = pd.DataFrame({"key": list(obj.keys()), "value": [to_cell(v) for v in obj.values()]})
    ...
    for idx, p in enumerate(selected, start=1):
        ...
        def add_any_table(key: str, title: str):
            if key not in payload:
                return
            if add_docx_table_any(doc, payload.get(key), title=title):
                doc.add_paragraph("")
